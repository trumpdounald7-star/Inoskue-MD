        ...
        getJid: {
            value(sender) {
                ...
                for (let chat of Object.values(conn.chats)) {
                    ...
                    if (user) {
                        return conn.isLid[sender] = user?.jid || user?.id;
                    }
                }
                return sender;
            }
    ...
    async genOrderMessage(message, options) {
        ...
        sendFile: {
            ...
            async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
                ...
                if (fileSize >= 4000) throw new Error('File size is too big!')
                let opt = {}
...
msToDate: {
   async value (ms) {
          ...
          return days + " days " + hours + " Hour " + minutes + " Menutes";
          // +minutes+":"+sec;
        }
    },
	...
	let type = await fileTypeFromBuffer(buffer)
       let trueFileName = attachExtension ? (filename + '.' + type.ext) : filename
        ...
        sendListButton: {
        async value(jid, text, list, footer, options = [], quoted = fkon) {
...
conn.relayMessage(jid, msg.message, {
  messageId: msg.key.id,
       ...
       sendUrlButton: {
        async value(jid, text, row = [], footer, quoted = fkon) {
	let msg = generateWAMessageFromContent(jid, {
  viewOnceMessage: {
    message: {
        "messageContextInfo": {
...
conn.relayMessage(jid, msg.message, {
  messageId: msg.key.id,
        ...
        sendUrlImageButton: {
        async value(jid, text, row = [], footer, url, quoted = fkon) {
	let msg = generateWAMessageFromContent(jid, {
  viewOnceMessage: {
    message: {
        "messageContextInfo": {
...
conn.relayMessage(jid, msg.message, {
  messageId: msg.key.id,
        ...
        sendListImageButton: {
        async value(jid, text, list, footer, url, options = [], quoted = fkon) {
	let msg = generateWAMessageFromContent(jid, {
  viewOnceMessage: {
    message: {
        "messageContextInfo": {
...
END:VCARD`.trim()
        ...
        return await conn.sendMessage(jid, {
            contacts: {
                displayName: (contacts.length > 1 ? `2013 contact` : contacts[0].displayName) || null,
                contacts,
            }
        },
        {
...
async value (status) {
        ...
        // <iq to="s.whatsapp.net" type="set" xmlns="status" id="21168.6213-69"><status>"Hi, A IM SILANA AI "</status></iq>
    }
},
        sendStickerFromUrl: {
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        chat: {
            get() {
                ...
                const decoded = this.conn.decodeJid(this.key?.remoteJid || (senderKeyDistributionMessage && senderKeyDistributionMessage !== 'status@broadcast'))
                return this.conn.getJid(decoded)
            }
        },
        isGroup: {
            get() {
        ...
        sender: {
            get() {
                const decoded = this.conn.decodeJid(this.participant || this.key.participant || this.chat)
                return this.conn.getJid(decoded)
            },
            enumerable: true
        },
